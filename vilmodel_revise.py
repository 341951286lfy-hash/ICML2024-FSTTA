...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None):
        """
        hidden_states: (N, L_hidden, D)
        attention_mask: (N, H, L_hidden, L_hidden)
        """
...
class GlocalTextPathNavCMT(BertPreTrainedModel):
    def __init__(self, config):
        ...
        # ------------------------------------------------------------------
        # Adapter 模块：
        # 1) Env Adapter 用于每步 panorama 特征的环境适配；
        # 2) Scene Adapter 分别作用于 global/local 导航分支，用于场景适配。
        #
        # 这三个模块会在 agent_obj.py 中被单独解冻并优化，因此必须注册在
        # 主模型对象上，供外部通过 self.vln_bert.xxx 直接访问。
        # ------------------------------------------------------------------
        self.env_adapter = EnvAdapter(config.hidden_size)
        self.scene_adapter_g = SceneAdapter(config.hidden_size)
        self.scene_adapter_v = SceneAdapter(config.hidden_size)
        ...
        if config.glocal_fuse:
            self.sap_fuse_linear = ClsPrediction(
    ...
    def forward_panorama_per_step(
        ...
        按文档要求，Env Adapter 要插在视觉编码器最后一层输出之后，
        并在返回给后续 gmap / vp 构建之前完成残差适配：

            pano_embeds = pano_embeds + self.env_adapter(pano_embeds)

        这样初始时由于 adapter 最后一层是零初始化，不会破坏原主干能力；
        测试时再通过环境一致性损失逐步学到修正量。
        """
        device = view_img_fts.device
        has_obj = obj_img_fts is not None

        # 先把 view token 映射到 hidden_size 空间
        view_img_embeds = self.img_embeddings.img_layer_norm(
            self.img_embeddings.img_linear(view_img_fts)
        )

        # 如果当前任务包含 object token，则把 object feature 也映射到同一隐空间
        if has_obj:
            ...
            # 将 view token 与 object token 拼成当前 step 的 panorama token 序列
            img_embeds = []
            for view_embed, obj_embed, view_len, obj_len in zip(
        ...
        # 融合图像特征、位置特征、导航类型 embedding 和 token type embedding
        ...
        if self.img_embeddings.pano_encoder is not None:
            pano_embeds = self.img_embeddings.pano_encoder(
                pano_embeds,
                src_key_padding_mask=pano_masks.logical_not()
            )

        # ------------------------------------------------------------------
        # Env Adapter：
        # 放在 panorama 编码输出之后、后续图构建之前。这里采用残差接法，
        # 只输出修正量 delta，主干保持 x + delta。
        # ------------------------------------------------------------------
        pano_embeds = pano_embeds + self.env_adapter(pano_embeds)
    ...
    def forward_navigation_per_step(
        ...
        """单步 navigation 编码与动作打分。

        按文档要求，Scene Adapter 要插在：
        - global_encoder.encoder() 之后
        - local_encoder.encoder() 之后
        - 各类导航头（global/local/fuse/object head）之前

        也就是说，先得到融合后的 global/local token 表示，再做轻量残差场景适配，
        最后才送入动作打分头。
        """
        batch_size = txt_embeds.size(0)

        # --------------------------------------------------------------
        # Global branch：构造 graph-map token，并通过 global encoder 编码
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # Local branch：构造当前 viewpoint 候选 token，并通过 local encoder 编码
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # Scene Adapter：
        # 放在 global/local encoder 输出之后，导航头之前。
        # 残差接法保证初始时不破坏原分支表示。
        # --------------------------------------------------------------
        ...
        # --------------------------------------------------------------
        # 根据 global/local 的首 token 表示估计动态融合权重
        # --------------------------------------------------------------
        if self.sap_fuse_linear is None:
            fuse_weights = 0.5
        else:
            fuse_weights = torch.sigmoid(
                self.sap_fuse_linear(torch.cat([gmap_embeds[:, 0], vp_embeds[:, 0]], 1))
            )

        # Global action logits：对已访问节点和 padding 位置做 mask
        ...
        # --------------------------------------------------------------
        # Glocal fusion：
        # 1) stop 动作由 global/local 的 stop logit 共同决定；
        # 2) 对于未访问节点，把 local branch 对候选视角的分数映射回 global map。
        # --------------------------------------------------------------
        fused_logits = torch.clone(global_logits)
        fused_logits[:, 0] += local_logits[:, 0]
        ...
        # Object grounding 分支：只在 object token 上保留得分
