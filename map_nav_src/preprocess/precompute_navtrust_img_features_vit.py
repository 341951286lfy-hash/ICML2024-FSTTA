...
# 把 map_nav_src 加入 import 路径
THIS_DIR = Path(__file__).resolve().parent
REPO_ROOT = THIS_DIR.parent
sys.path.insert(0, str(REPO_ROOT / "map_nav_src"))
...
def render_36_views(sim, scan_id: str, viewpoint_id: str):
    """
    返回 list[np.ndarray]，长度 36，每个元素是 RGB uint8 图像。
    """
    images = []
...
def extract_features(model, transform, images_rgb, device: str, batch_size: int):
    ...
    for st in range(0, x.size(0), batch_size):
        ...
        if isinstance(fb, (tuple, list)):
            fb = fb[0]
        if fb.ndim == 3:          # [B, N, C]，取 CLS token
            fb = fb[:, 0]
...
def main():
    ...
    severity_tag = str(args.severity).replace(".", "p")
    print(f"[INFO] corruption={args.corruption}, severity={args.severity}, tag=s{severity_tag}")
