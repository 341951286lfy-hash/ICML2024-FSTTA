...
def build_dataset(args, rank=0):
    ...
    if args.aug is not None:
        ...
        aug_env = dataset_class(
            ...
            sel_data_idxs=None,
            name='aug',
            multi_endpoints=args.multi_endpoints,
...
def train(args, train_env, val_envs, aug_env=None, rank=-1):
    ...
    listener = GMapObjectNavAgent(args, train_env, rank=rank)
    ...
    best_val = {'val_unseen': {"spl": 0.0, "sr": 0.0, "state": ""}}
    ...
    for idx in range(start_iter, start_iter + args.iters, args.log_every):
        ...
        if aug_env is None:
            ...
            listener.train(interval, feedback=args.feedback)
        else:
        ...
        if default_gpu:
            total = max(sum(listener.logs['total']), 1)
            length = max(len(listener.logs['critic_loss']), 1)
            critic_loss = sum(listener.logs['critic_loss']) / total
            policy_loss = sum(listener.logs['policy_loss']) / total
            og_loss = sum(listener.logs['OG_loss']) / max(len(listener.logs['OG_loss']), 1)
            ...
            write_to_record_file(
                ...
                "OG_loss %.4f, policy_loss %.4f, critic_loss %.4f" % (
            ...
            for env_name, env in val_envs.items():
                ...
                if env_name in best_val and score_summary['spl'] >= best_val[env_name]['spl']:
                    best_val[env_name]['spl'] = score_summary['spl']
                    best_val[env_name]['sr'] = score_summary['sr']
                    best_val[env_name]['state'] = 'Iter %d %s' % (iter_, loss_str)
                    listener.save(idx, os.path.join(args.ckpt_dir, "best_%s" % env_name))

            listener.save(idx, os.path.join(args.ckpt_dir, "latest_dict"))
            write_to_record_file(
                '%s (%d %d%%) %s' % (
                    timeSince(start, float(iter_) / args.iters),
                    iter_,
                    float(iter_) / args.iters * 100,
                    loss_str,
                ),
                record_file,
            )
...
def valid(args, train_env, val_envs, rank=-1):
    ...
    agent = GMapObjectNavAgent(args, train_env, rank=rank)
    ...
    for env_name, env in val_envs.items():
        prefix = 'submit' if not args.detailed_output else 'detail'
